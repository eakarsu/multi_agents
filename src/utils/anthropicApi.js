...
// Uses backend proxy to avoid CORS issues with Anthropic API
...
class AnthropicAPI {
  ...
  async sendMessage(messages, systemPrompt = '', model = 'claude-3-5-sonnet-20241022', maxTokens = 1000) {
    try {
  ...
  async customerSupportAgent(customerQuery, customerContext = {}) {
    try {
  ...
  async contentCreationAgent(contentType, topic, audience, tone = 'professional') {
...
4. Has a clear call-to-action`;
    ...
    return await this.sendMessage(messages, systemPrompt, 'claude-3-sonnet-20240229', 1500);
  }

  // Lead Qualification Agent
  async leadQualificationAgent(leadData) {
...
Provide specific reasoning and recommended actions.`;
  ...
  // Data Analysis Agent
  async dataAnalysisAgent(dataDescription, analysisType, businessGoals) {
...
// Demo mode fallback responses when API is not available
