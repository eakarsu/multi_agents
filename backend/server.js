...
app.post('/api/agents/content-creation', async (req, res) => {
  try {
    ...
    res.status(500).json({ 
      error: 'Failed to process content creation request',
      message: error.message 
    });
  }
});
...
app.post('/api/agents/lead-qualification', async (req, res) => {
  try {
    ...
    res.status(500).json({ 
      error: 'Failed to process lead qualification request',
      message: error.message 
    });
  }
});
...
// Generic Agent endpoint - Can handle any agent template
app.post('/api/agents/generic', async (req, res) => {
  try {
    ...
    if (!agentType || !agentName || !task) {
      return res.status(400).json({ 
        error: 'agentType, agentName, and task are required',
        received: { agentType, agentName, task }
      });
    }
    ...
    res.status(500).json({ 
      error: 'Failed to process generic agent request',
      message: error.message 
    });
  }
});
...
app.listen(PORT, () => {
  ...
  console.log(`🤖 Anthropic API proxy: http://localhost:${PORT}/api/anthropic/messages`);
  console.log(`💬 Customer Support: http://localhost:${PORT}/api/agents/customer-support`);
  console.log(`✍️  Content Creation: http://localhost:${PORT}/api/agents/content-creation`);
});
